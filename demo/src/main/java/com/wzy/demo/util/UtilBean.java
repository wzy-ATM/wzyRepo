...
public class UtilBean {
    ...
    //spring依赖注入即在容器中实例化对象bean，依赖注入的主要目的,是让容器去产生一个对象的实例,然后在整个生命周期中使用他们
    //而statis是类属性，不会实例化对象，所以不能使用@Autowired注入静态变量
    //所以这种方式是错误的，肯定是空指针的
    //@Autowired
    //private static LuckMoneyService luckMoneyService;


    //解决方法1如下：将@Autowire加到构造方法上，将spring实例化的对象赋值给静态变量
    private static LuckMoneyService luckMoneyService;
    @Autowired
    ...
    @Autowired
    private LuckMoneyService luckMoneyService2;
    @PostConstruct
    public void beforeInit() {
        LuckMoneyService1 = luckMoneyService2;
    }
